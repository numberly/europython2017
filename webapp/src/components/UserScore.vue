<template>
  <div class="card-user">
    <span class="card-user_name">
      <flag :iso="getFlag" style="margin-right: 10px"/>
      {{user.name}}
    </span>
    <span class="card-user_score">
      <span v-if="datetime">{{user.scores[datetime]}}</span>
      <span v-if="!datetime">{{user.total_score}}</span>
      <span></span>
...
export default {
  ...
  methods: {
    querySearch (queryString) {
      ...
      return queryString ? countries.filter(this.createFilter(queryString)) : countries
    },
  ...
  computed: {
    getFlag () {
      return this.querySearch(this.user.country)[0].code
    }
  }
}
</script>
